...
class PlayerSelectionGUI:
    def __init__(self, master):
        ...
        self.confirm_button = customtkinter.CTkButton(master, text="Start Game", corner_radius=5, command=self.start_game)
...
class TicTacToeGUI:
    def __init__(self, master, player_choice):
        ...
        for i in range(3):
            for j in range(3):
                btn =  customtkinter.CTkButton(master, text=' ', width=10, height=3,
                                command=lambda row=i, col=j: self.on_click(row, col), corner_radius=10)
    ...
    def on_click(self, row, col):
        ...
        if self.board[index] == ' ':
            ...
            self.buttons[index].configure(text=self.player)  # Corregir config a configure
    ...
    def make_computer_move(self):
        ...
        self.buttons[position].configure(text=self.player)  # Corregir config a configure
        winner = check_winner(self.board)
        if winner != ' ':
            messagebox.showinfo("Winner", f"{winner} wins!")
            self.reset_board()
    ...
    def reset_board(self):
        ...
        for btn in self.buttons:
            btn.configure(text=' ')  # Corregir config a configure
